...
// - Screen support added for mouse scroll 
// ==================================================
...
const unsigned long gripDebounceDelay = 80; // Mouse Debouncing
...
void RecordFunction() {
  ...
  while (digitalRead(buttonPin) == LOW); // keep for hardware safety
...
// ================= MOUSE ==========================
// ==================================================
void mouse_control(int8_t dx, int8_t dy, int8_t scroll, uint8_t buttons) {
  if (estopActive) return;
  ...
  if (menuMode) {
    ...
    // Left click = select
    if ((buttons & 0x01) && millis() - lastButtonTime > buttonDelay) {
      ...
      if (inMenuScreen) {
        ...
        switch (menuIndex) {
          ...
          case 1: ReturnHome(home_x, home_y, home_z); break;
          case 2: bring_cup(); break;
          case 3: elevator_setup(); break;
          //case 4: currentFunction = 0; playCustomFunction(); break;
          //case 5: currentFunction = 1; playCustomFunction(); break;
          //case 6: currentFunction = 2; playCustomFunction(); break;
          //case 7: selectFunctionToRecord(); break;
          //case 8: renameCustomFunction(); break;
        }
    ...
    return;  // IMPORTANT: stop normal mouse control when in menu
  }
  if (!gripperMode) {
    // ===== XYZ CONTROL =====
    float newX = curxpos - dx * 0.04;
    float newY = curypos - dy * 0.04;
    float newZ = curzpos + scroll * 5;

    newX = constrain(newX, X_MIN, X_MAX);
    newY = constrain(newY, Y_MIN, Y_MAX);
    newZ = constrain(newZ, Z_MIN, Z_MAX);

    clampXYZToSphere(newX, newY, newZ);

    if (newX != curxpos || newY != curypos || newZ != curzpos) {
  ...
  if (buttons & 0x04) {
    ...
    if (gripperMode) drawGripperScreen();
    else drawArmManualScreen();
  }
  // Left click detection (edge-triggered)
  static bool lastMouseLeft = false;
  bool currentMouseLeft = (buttons & 0x01);

  if (currentMouseLeft && !lastMouseLeft && millis() - lastButtonTime > buttonDelay) {
    mouseClick = true;
    lastButtonTime = millis();
  }
...
lastGripButtonState = currentGripButtonState;
  ...
  menuMode = (digitalRead(switchPin) == LOW);
...
extern "C" void tuh_hid_report_received_cb(uint8_t dev_addr, uint8_t instance,
  ...
  if (tuh_hid_interface_protocol(dev_addr, instance) == HID_ITF_PROTOCOL_MOUSE && len >= 3) {

    mouseActive = true;

    uint8_t buttons = report[0];
    int8_t dy = (int8_t)report[1];
    int8_t dx = (int8_t)report[2];
    int8_t scroll = (len >= 4) ? (int8_t)report[3] : 0;

    mouse_control(dx, dy, scroll, buttons);
  }
